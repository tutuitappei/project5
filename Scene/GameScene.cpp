...
GameScene::GameScene()
{
	...
	for (int y = 0; y < 5; y++)
	{
		for (int x = 0; x < 10; x++)
		{
			...
			if ((((x + y * 10) / 2) % 3) != 2)
			{
				tmpMoveState.emplace_back(MOVE_TYPE::SIGMOID, Vector2db{ (y % 2)*-200.0 + lpSceneMng.GameScreenSize.x / 2 + lpSceneMng.GameScreenOffset.x - 48, lpSceneMng.GameScreenSize.y *(4.0 / 7.0) + lpSceneMng.GameScreenOffset.y});
			}
			else
			{
				tmpMoveState.emplace_back(MOVE_TYPE::SIGMOID, Vector2db{ (y % 2)*-200.0 + lpSceneMng.GameScreenSize.x / 2 + lpSceneMng.GameScreenOffset.x - 48, lpSceneMng.GameScreenSize.y*(4.0 / 7.0) - (32 * 4) + lpSceneMng.GameScreenOffset.y});
			}

			tmpMoveState.emplace_back(MOVE_TYPE::SPIRAL, Vector2db{ (y % 2)*-200.0 + lpSceneMng.GameScreenSize.x / 2 + lpSceneMng.GameScreenOffset.x - 48, lpSceneMng.GameScreenSize.y*(6.0 / 7.0) - 32 * 2 - 8});
			...
			EnemyState dete = { ENEMY_TYPE::C ,{ static_cast<double>(lpSceneMng.GameScreenSize.x * (((y*10)+x)%2)),
